...
- https://spring.io/blog/2026/06/06/spring-ai-2-0-0-RC1-available-now
...
### Erupt
- **Badge:** Framework
- **Description:** Annotation-driven Java admin framework (Spring Boot 3) with a built-in, fully UI-configurable AI harness. Annotate a JPA entity to generate a complete admin page, then wire LLMs and tools with no code: 50+ LLM providers, MCP-native tool calling (SSE & STDIO), A2A multi-agent workflows, and per-role tool access — all built on LangChain4j. Expose Spring beans as AI-callable tools via `@AiToolbox`. Apache-2.0.
- **Links:** [GitHub](https://github.com/erupts/erupt) · [Docs](https://docs.erupt.xyz) · [Demo](https://demo.erupt.xyz)
