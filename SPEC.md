...
- **Badge:** Framework
- **Description:** Official Micronaut integration for LangChain4j. Compile-time-generated AI services with modules for OpenAI, Anthropic, Azure, Bedrock, Google AI, and Mistral, plus embedding stores for PostgreSQL, Redis, MongoDB, Elasticsearch, Neo4j, Chroma, and Qdrant. Version 2.2.0 adds agentic support, Chroma embedding stores, and guardrails resolved from Micronaut beans.
- **Links:** [Docs](https://micronaut-projects.github.io/micronaut-langchain4j/latest/guide/) · [GitHub](https://github.com/micronaut-projects/micronaut-langchain4j)
...
### Spring AI A2A
- **Badge:** SDK
- **Description:** Spring Boot integration for building Agent2Agent (A2A) protocol servers. Auto-configured JSON-RPC endpoints and task lifecycle handling wired to Spring AI's `ChatClient`, with full `@Tool` support and multi-agent orchestration. Complements the A2A Java SDK for Spring applications.
- **Links:** [GitHub](https://github.com/spring-ai-community/spring-ai-a2a)

### Spring AI Session
- **Badge:** Library
- **Description:** Event-sourced conversation memory for Spring AI. Structured events with identity, timestamps, and session ownership; turn-aware compaction that trims history on turn boundaries; multi-agent branch isolation; keyword search over history; and pluggable in-memory or JDBC repositories via an SPI.
- **Links:** [Docs](https://spring-ai-community.github.io/spring-ai-session/) · [GitHub](https://github.com/spring-ai-community/spring-ai-session)
...
- **Badge:** SDK
- **Description:** Java SDK for driving the Claude Code CLI as a subprocess, with three APIs — one-shot `Query`, blocking multi-turn `ClaudeSyncClient`, and reactive `ClaudeAsyncClient` (Project Reactor). Supports hooks, permission callbacks, MCP server integration, and subagent definitions, with no native dependencies. Created by Spring AI project lead Mark Pollack, continuing development from the archived `spring-ai-community/claude-agent-sdk-java`. Complements [Agent Client](#agent-client), which targets multiple CLI coding agents instead of just Claude Code. Apache 2.0.
- **Links:** [Docs](https://lab.pollack.ai/projects/claude-agent-sdk) · [GitHub](https://github.com/markpollack/claude-agent-sdk-java)
...
### inference4j
- **Badge:** Inference
- **Description:** Inference-only library wrapping ONNX Runtime behind typed Java APIs for image classification, object detection, CLIP, speech-to-text, embeddings, reranking, NER, and text generation — taking standard Java types rather than tensors. Downloads and caches models from Hugging Face automatically, and ships a Spring Boot starter. Also publishes the only Maven Central build of Microsoft's onnxruntime-genai Java bindings.
- **Links:** [GitHub](https://github.com/inference4j/inference4j) · [Docs](https://inference4j.github.io/inference4j)
